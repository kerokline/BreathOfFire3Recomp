...
#
# Writes: dist/bof3-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix bof3 \
  --exe-name BreathOfFire3_Recompiled \
  --display-name "BreathOfFire3 Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned BreathOfFire3 disc" \
