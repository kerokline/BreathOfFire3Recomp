...
eight 0x88-byte species records (slots 0..7, the byte an enemy's working
record keeps at +0x60 and its object at +0xE0 -- the index of its AI script
row AND of its bank-6 creature cue, docs/SOUND_CUES.md), then 0x48 bytes of
zero. A record:
...
def cmd_show(a):
    ...
    g = gloss()
    for k, d in tables(a.bin_root).get(n, []):
