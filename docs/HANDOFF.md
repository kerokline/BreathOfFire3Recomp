...
| `tools/enemy_table.py` | **Every area's enemy species table off the disc**: the `0x800E4000` section of each `AREAnnn.EMI`, 8 records with the 8-byte name at `+0x48` and stats at `+0x54`; `extract` → `names/enemies.toml` (448 rows, 168 species) merging English names from `names/enemy_gloss.toml` (seeded from the BoF wiki, 167/168), `show AREA048`, `species` |
