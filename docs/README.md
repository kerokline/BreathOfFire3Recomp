...
| [`FURIGANA.md`](FURIGANA.md) | built + evidence | **Japanese (Ruby)**, the third selectable script (`jp_ruby`, built 2026-09-09 by `tools/build_ruby_script.py`, on screen): why inline rather than rendered ruby, the 15-glyph box line, what annotating costs in rows over all 3,793 confirm pages, the re-authoring rules, the byte budget against the 16 KiB window, and the next session's build order — `tools/ruby_fit.py` |
