...
| `names/*_records.toml` | **engine loader entries** — the pc the engine `jalr`s into per area / chapter / boss / party combo / ability, written by `tools/loader_records.py` and `tools/magic_map.py`, consumed by `extract_overlays.py` (`ENGINE_RECORD_FILES`) and `tools/xref.py`. Generated; never hand-edited — see [`LOADER_RECORDS.md`](LOADER_RECORDS.md) | (`section` md5, `entry` pc) |
