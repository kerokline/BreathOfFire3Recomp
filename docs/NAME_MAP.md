...
| `names/se_cues.toml` | the **sound catalogue**: one row per SPU sample (md5 of the bytes), `label` heard by the player, `auto` = owner file + VAG number, every `cue@context` it arrived through, disc homes (`tools/se_watch.py`, `tools/audio_banks.py`; SOUND_CUES.md) | sample md5 |
| `names/audio_banks.toml` | every **(6, 8, 7) audio triplet** on the disc decoded: bank, cue entries → VAB program/tone → VAG, sizes, md5 — generated by `tools/audio_banks.py index`, never hand-edited | file + bank |
| `names/enemies.toml` | every **area's enemy species table** off the JP disc (name at record `+0x48`, stats at `+0x54`), with the US disc's own 8-character card (`us`) and the wiki's full name (`en`) — generated by `tools/enemy_table.py extract`, never hand-edited | area + slot |
| `names/enemy_gloss.toml` | hand-editable **English names per Japanese enemy name** (seeded from the BoF wiki; `se_watch --label` adds to it); merged into `enemies.toml`'s `en` | `jp` |
