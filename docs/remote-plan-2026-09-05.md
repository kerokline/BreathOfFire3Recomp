...
**Status:** IN PROGRESS (written 2026-09-05 08:10; A–F done by ~12:30 — see Outcomes)
...
| B | **DONE 2026-09-05 ~11:45.** Every field the `battlebegin.json` trace saw at +563 is a store in the body: the effective-stat block `C+0x90..+0xAC` is snapshotted to `C+0xB0..+0xCC`, `Formation_ApplyStatMods` (engine `0x800A79AC`, on `0x80144F54`) adjusts the scratch, and it is copied back; status `0x2000` = low HP (< max/4) set here; flags, command, `+0x121`, `+0x12C..+0x133`, `+0x136..+0x139` zeroed; battle globals reset (turn counter 1, totals, escape attempts, drop count). The four callees: `0x800A79AC` = formation mods, `0x800A8AD4` = normal/boss split (both named, hypothesis), `0x8014E294(0x1A/0x1B)` and `0x801629CC(sound)` boot helpers left unnamed. `Battle_Init` promoted to **evidence**; actor-table "effective stats" rows added to BATTLE_RAM.md. |
| C | **DONE 2026-09-05 ~10:15.** Five commands captured from `slot03` (Attack / Defend / Watch / Auto / Run ×3, all Runs succeeded) plus `slot02` Nu and a victory capture that paged through the results. Venn result: no command has its own compiled function (0 auto-only, 0 defend-only; watch-only = Examine state entries; run-only = the escape exit). Found instead: `Battle_BuildTurnOrder` (party AGI `+0x24` + command bonus × level-class pct; enemy AGI `+0x28` + random) writing the list at `0x80146308`, `Battle_EnemyDefeated` (enemy `+0x04` zenny / `+0x06` EXP yields, `+0x08` = level), `Battle_RollDrops` (drop slots `+0x18..+0x1F`, threshold table `0x801D0CB8`), the BATL_END tally (`BattleResult_Setup` / `_ExpTick` / `_ZennyTick` / `_AwardDrops` = the `Inventory_Add` caller) and the banner pool. 30 names (17 evidence). Enemy skill damage goes through engine `0x8009A3E4`, not `Battle_ApplyDamage`. No drop landed (1.6 % / 0.4 %). BATL_END runs mostly interpreted — harvested (6 sessions, 50.8 % est. coverage). [`BATTLE_RAM.md`](BATTLE_RAM.md) "Turn order, kills, drops and the results tally". **Addendum ~10:45:** the command handlers were in the engine band outside the filter; `e_*.json` captures with `--lo 0x80093800` found `Cmd_ConfirmAttack` / `Cmd_ConfirmWatch`, `Escape_Roll` + `Escape_Chance` (AGI-difference table `0x800B1430`, certain from the 3rd attempt), Auto's flag/fill/refill, `Battle_CommitRound` and the table-driven `EnemyAI_ChooseActions`. 19 more names. |
| D | **DONE 2026-09-05 ~11:30.** Two fresh `build-dbg` boots harvested during the intro with session ids `d_boot1` / `d_boot2` and area `LOGO/LOGO.EXE`: **0 new proven entries and 0 interpreted PCs in the `0x801CE000` band on either boot** — the logo band is fully native since the 2026-09-01 handler harvest, so `pc_coverage.py` keeps printing NEVER SAMPLED for it (an estimator needs rows; a band that produces none is saturated, not unsampled — the tool should say so). Boot-wide interp ≈ 0.17 M insns/s cumulative. Perf half: `build-relprof` rebuilt against `17f49ad3` (ninja: nothing to do — the 08:17 exe already matched), `fmv_bench.py` on it: **99.6 present/s, 100.1 vblank/s, 38.6 k interp insns/s** in the FMV window vs the 2026-09-01 post-fix baseline of ~97.6 (uncapped, so >60 is the headroom, not a pacing fault); `build-dbg` 50.1/s (baseline 49–52). No regression from the pin. |
| E | **DONE 2026-09-05 ~12:15.** `tools/save_tool.py` (`list` / `dump` / `verify` / `diff`) over raw card images. `card1.mcd`: checksum OK on all three saves; every slot-summary field the load screen reads matches the block it was copied from (name, party ids with the Peco rule, level, play time, EXP, flag bit) and the SJIS title's hours/minutes/level agree. Oracle: Mednafen booted from the same card, load screen = リュウ Lv 1, 00:28 / 05:13 / 05:10, portraits Ryu alone / Teepo-Rei-Ryu ×2 — identical to the tool (`analysis/mednafen_loadscreen_card1.png`). `diff 2 3`: only checksum + play time differ, as predicted. Corrections to BATTLE_RAM: `0x8015BFC4` is `Flag_Test` (bit `0x92`, not a hash); play time = `0x80144FBC` h/m/s/f; abilities = four 10-slot lists `+0x5C/+0x66/+0x70/+0x7A` (`AbilityList_ForType`). Roster order fully read from the record names (target 6 closed). |
| F | **DONE 2026-09-05 ~12:30** (go-ahead given at the desk). Watchdog patch applied on fork branch `fix/starvation-watchdog-wrap` (`430c93b8`, = `17f49ad3` + 1): heartbeat loaded before the clock via the pure `starvation_watchdog_stale()`, `_Atomic` heartbeat, exit origins `starvation_watchdog` / `console_close` / `console_ctrl_*` / `signal_sig*`, ctest `starvation_watchdog_test` (11 checks). Verified on `build-dbg`: 10 min with the poller at 1 s and the 4 s watchdog **enabled** — alive, no dump (pre-fix: ~2 min to trip); `PSX_STARVATION_TIMEOUT_US=1000` trips at boot and the report reads `exit_origin: starvation_watchdog`. Filed as [mstan/psxrecomp#321](https://github.com/mstan/psxrecomp/pull/321). Submodule returned to the `17f49ad3` pin; `setx` workaround stays until the bump. `build-relprof` was already current (D). |
